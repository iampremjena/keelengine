...
export default function Dashboard({ session }) {
  ...
  const fetchResults = async (e) => {
    ...
    try {
      ...
      // Protect against Render HTML Wakeup Error
      if (!res.ok) throw new Error("The server is waking up. Please wait 10 seconds and try again.");
  ...
  const saveNeighborhoodToDB = async (hub) => {
    ...
    try {
      const { error } = await supabase.from('saved_properties').insert([{ user_id: session.user.id, neighborhood: hub.Neighborhood, outcode: hub.Station_Outcode, rent_range: hub.Rent_Range, suggestion_score: hub.Suggestion_Score }]);
      if (error) throw error;
      showAlert("Saved!", `${hub.Neighborhood} added to your Profile.`, "success");
    } catch (e) {
      ...
      <div className={`flex flex-col lg:flex-row gap-8 transition-all duration-700 ease-in-out ${!hasSearched ? 'justify-center items-center' : 'items-start'}`}>
        ...
        {hasSearched && (
          <div className="w-full lg:w-2/3 animate-fadeIn space-y-6">
            {loading && <div className="glass rounded-3xl py-32 text-center"><div className="w-10 h-10 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div><p className="text-slate-400">Searching London for your perfect neighborhoods...</p></div>}
            ...
            {!loading && currentItems.map((hub, idx) => (
              <div key={idx} className="glass rounded-3xl p-6 shadow-xl border border-slate-700/40">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-2 text-center"><span className="block text-[10px] text-slate-400 uppercase font-bold">Match Score</span><span className="text-xl font-black text-emerald-400">{hub.Suggestion_Score}</span></div>
                </div>
                
                <NeighborhoodMap lat={hub.Latitude} lng={hub.Longitude} />

                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                  <div className="bg-slate-900/40 p-3 rounded-xl"><span className="block text-[10px] text-slate-400">Commute Time</span><strong className="text-white">{hub.Commute_Duration} mins</strong></div>
                  <div className="bg-slate-900/40 p-3 rounded-xl"><span className="block text-[10px] text-emerald-400">Rent Cost</span><strong className="text-emerald-400">{hub.Rent_Range}</strong></div>
                  <div className="bg-slate-900/40 p-3 rounded-xl"><span className="block text-[10px] text-blue-400">Transit Pass</span><strong className="text-blue-400">{hub.Single_Fare_Cost}</strong></div>
                  <div className="bg-slate-900/40 p-3 rounded-xl"><span className="block text-[10px] text-amber-400">Safety Rating</span><strong className="text-amber-400">{hub.Safety_Score}/100</strong></div>
                </div>
                ...
                <div className="flex gap-4 items-center">
                  <a href={`https://www.google.com/maps/dir/?api=1&origin=${hub.Latitude},${hub.Longitude}&destination=${postcode.toUpperCase()}&travelmode=transit`} target="_blank" className="w-2/3 bg-blue-600 hover:bg-blue-500 rounded-xl py-3 text-center text-xs font-bold text-white transition">🗺️ See Commute on Google Maps</a>
                  <button onClick={() => saveNeighborhoodToDB(hub)} className="w-1/3 bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 rounded-xl text-xs transition border border-slate-600 shadow-lg">❤️ Save</button>
                </div>
              </div>
            ))}
            ...
            {!loading && results.length > 0 && (
              <div className="flex justify-center items-center gap-6 mt-8 pb-10">
                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-5 py-2 bg-slate-800 disabled:opacity-30 rounded-xl text-white font-bold border border-slate-700 hover:border-emerald-500 transition">← Back</button>
