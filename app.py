        ...
        for _, row in df_hubs.iterrows():
            ...
            with st.container():
                ...
                with col_actions:
                    ...
                    # Map configurations to clear, natural search phrase parameters
                    term_map = {
                        "Shared Flatshare / Room": "flatshare",
                        "1-Bed Private Flat": "1 bed flat",
                        "2-Bed Private Flat": "2 bed flat",
                        "3-Bed Private Flat": "3 bed flat"
                    }
                    search_term = term_map.get(property_input, "flat")
                    
                    # FIX: Force exact-match filtering by enclosing the targeted outcode string in strict quotes
                    query_text = f'"{outcode_target}" {search_term} under {int(rent_share)}'
                    encoded_query = urllib.parse.quote(query_text)
                    google_search_url = f"https://www.google.com/search?q={encoded_query}"
                    
                    st.link_button(f"Find Listings in {outcode_target} ➔", google_search_url, use_container_width=True)
